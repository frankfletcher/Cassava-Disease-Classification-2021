...
def get_train_aug(RESOLUTION=380): 
    ...
    return A.Compose(
        [
            A.SmallestMaxSize(
                max_size=RESOLUTION * 2,
                interpolation=cv2.INTER_CUBIC,
                always_apply=True,
            ),
            A.PadIfNeeded(
                min_height=RESOLUTION * 2,
            ...
            A.Resize(
                RESOLUTION, RESOLUTION, p=1.0, interpolation=cv2.INTER_CUBIC
            ),
            A.FancyPCA(p=0.8, alpha=0.5),
            # #         A.Transpose(p=0.7),
            A.HorizontalFlip(p=0.5),
            ...
            A.CoarseDropout(
                p=0.8,
                max_holes=30,
                min_height=int(RESOLUTION / 180),
                max_height=int(RESOLUTION / 90),
                min_width=int(RESOLUTION / 180),
                max_width=int(RESOLUTION / 90),
                fill_value=[int(i / 3) for i in get_rand_dropout_color()],
            ),
            # #         A.Cutout(p=0.8, max_h_size=40, max_w_size=40),
            A.Cutout(
                p=1,
                max_h_size=int(RESOLUTION / 12),
                max_w_size=int(RESOLUTION / 20),
                num_holes=16,
                fill_value=get_rand_dropout_color(),
            ),
            A.Cutout(
                p=1,
                max_h_size=int(RESOLUTION / 20),
                max_w_size=int(RESOLUTION / 12),
                num_holes=16,
                fill_value=get_rand_dropout_color(),
            ),
            A.OneOf(
                [
                    A.OpticalDistortion(
                        ...
                        shift_limit=(-0.6699999570846558, 0.4599999785423279),
                        interpolation=0,
                        border_mode=0,
                        value=(0, 0, 0),
                        mask_value=None,
                    ...
                    A.GridDistortion(
                        ...
                        p=1.0,
                        num_steps=6,
                        distort_limit=(-0.4599999785423279, 0.5),
                        interpolation=0,
                        border_mode=0,
                        value=(0, 0, 0),
                        mask_value=None,
            ...
            A.OneOf(
                [
                    A.Equalize(
                        always_apply=False, p=0.9, mode='cv', by_channels=True
                    ),
                    # #             A.Solarize(always_apply=False, p=0.9, threshold=(67, 120)),
                    # #             A.IAAAdditiveGaussianNoise(p=1.0),
                    A.GaussNoise(p=0.9),
                    A.MotionBlur(
                        always_apply=False, p=0.9, blur_limit=(4, 10)
                    ),
                    A.ISONoise(
                        always_apply=False,
                        p=0.9,
